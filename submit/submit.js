document.addEventListener('DOMContentLoaded', () => {
    ...
    function addCollaborator() {
        ...
        box.innerHTML = `
            ...
            <div class="collab-fields-grid">
                <div class="collab-field">
                    <label class="collab-field-label">Artist Name (Stage Name) <span class="req-star" style="color: var(--required-star, #ff4d6d);">*</span></label>
                ...
                <div class="collab-field" style="grid-column: 1 / -1;">
                    <label class="collab-field-label">Email Address <span class="req-star" style="color: var(--required-star, #ff4d6d);">*</span></label>
                    <input type="email" class="collab-field-input collab-email" placeholder="e.g. collaborator@gmail.com" required>
                </div>
                <div class="collab-field" style="grid-column: 1 / -1;">
        ...
        // Trigger hydration progress whenever collaborator inputs are typed in
        box.querySelectorAll('input').forEach(inputEl => {
            inputEl.addEventListener('input', () => updateWaterLevel());
        });
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            // Check collaborators: If any collaborator box is added, Artist Name and Email Address are strictly MANDATORY!
            const collabBoxes = collaboratorsList.querySelectorAll('.collaborator-box');
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            for (let i = 0; i < collabBoxes.length; i++) {
                const cBox = collabBoxes[i];
                const cArtist = cBox.querySelector('.collab-artist')?.value.trim();
                const emailInp = cBox.querySelector('.collab-email');
                const cEmail = emailInp?.value.trim();

                if (!cArtist) {
                    cBox.classList.remove('input-error-shake');
                    void cBox.offsetWidth;
                    cBox.classList.add('input-error-shake');
                    cBox.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    cBox.querySelector('.collab-artist')?.focus();
                    await showInSiteAlert({
                        title: 'Collaborator Name Required',
                        message: `Please provide the Artist Name for Collaborator ${i + 1}.`,
                        type: 'warning'
                    });
                    return;
                }
                ...
                if (!cEmail) {
                    ...
                    emailInp?.focus();
                    await showInSiteAlert({
                        title: 'Collaborator Email Required',
                        message: `Please enter an official contact email address for Collaborator ${i + 1} (${cArtist}). Email is mandatory for each collaborator.`,
                        type: 'warning'
                    });
                    return;
                }
                ...
                if (!emailRegex.test(cEmail)) {
                    ...
                    emailInp?.focus();
                    await showInSiteAlert({
                        title: 'Invalid Collaborator Email',
                        message: `Please enter a valid email address for Collaborator ${i + 1} (${cArtist}).`,
                        type: 'warning'
                    });
                    return;
                }
            ...
            collabBoxes.forEach(box => {
                ...
                const cEmail = box.querySelector('.collab-email')?.value.trim();
                ...
                if (artist || real || cEmail || role || spotify || apple) {
                    collaborators.push({
